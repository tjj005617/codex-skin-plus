// Wallpaper theme injector via CDP - supports both video and image
const fs = require("fs");
...
async function injectWallpaper(cdpPort) {
  ...
  const imagePath = path.join(ACTIVE_THEME_DIR, theme.image);
  const cssPath = path.join(ENGINE_DIR, "dream-skin.css");
  
  // Check if files exist
  if (!fs.existsSync(imagePath)) {
    return { error: `File not found: ${theme.image}` };
  }
  ...
  for (let i = 0; i < imageBase64.length; i += chunkSize) {
    await send("Runtime.evaluate", {
      expression: `window.__DV__.push(${JSON.stringify(imageBase64.slice(i, i + chunkSize))});`,
    });
  }
  ...
  const result = await send("Runtime.evaluate", {
    expression: `(async () => {
      try {
        ...
        // Cleanup
        ["codex-dream-skin-video","codex-dream-skin-image","codex-dream-skin-style","codex-dream-skin-chrome"].forEach(id => {
...
if (process.argv[1] && process.argv[1].includes("inject-theme")) {
  const port = process.argv[2] || "30123";
  injectWallpaper(parseInt(port)).then(r => {
    console.log(JSON.stringify(r, null, 2));
    process.exit(r?.ok ? 0 : 1);
  }).catch(e => { console.error(e.message); process.exit(1); });
}

module.exports = { injectWallpaper };
